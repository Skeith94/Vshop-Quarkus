...
public class UserController {
    ...
    public Uni<Response> login(@RequestBody @Valid LoginRequest request) {
        return Panache.withTransaction(()->userService.getByEmail(request.getEmail()))
                .onItem()
                .ifNull()
                .failWith(new WebApplicationException("wrong email"))
                .onItem()
                .transform(logged -> {
    ...
    @POST
    @Path("/ban")
    public Uni<Response> banUser(@RequestBody @Valid BannRequest request)  {
    return  Panache.withTransaction(()->userService.getByEmail(request.getEmail()))
            .onItem()
            .ifNull()
            .failWith(new WebApplicationException("wrong email"))
    ...
    public Uni<Response> createRole(@Size(min = 1, max = 15) @RestQuery String role){
        return Panache.withTransaction(()->Role.findByName(role))
                .onItem()
                .ifNotNull()
                .failWith(new WebApplicationException("role already exist"))
                .onItem()
                .transform(empty->new Role(role)).onItem()
                .transformToUni(roleToADD->Role.persist(roleToADD))
                .onItem()
    ...
    public Uni<Response> addRole(@PathParam("userId") Long userId, @PathParam("roleId") Long roleId) throws SystemException, InterruptedException {
       ...
       Uni<User>user=Panache.withTransaction(()->userService.findById(userId))
               .onItem()
               .ifNull()
               .failWith(new WebApplicationException("user id no present"));

       Uni<Role>role=Panache.withTransaction(()->Role.findById(roleId))
               .onItem()
               .ifNull()
               .failWith(new WebApplicationException("role id no present"));
       ...
       .onItem()
               .transformToUni(userUp->userService.persistFlushUser(userUp))
       .onItem()
               .transform(
                       userFinish->Response.ok("user:"+userFinish.getEmail()+" role:"+userFinish.getRole().getName()+" updated").type(TEXT_PLAIN_TYPE).build());
    }
    ...
    public Uni<Response> register(@RequestBody @Valid RegistrerRequest request) throws ExecutionException, InterruptedException {
        ...
        Uni<Country>countryCheck= Panache.withTransaction(()->Country.findById(request.getCountry_id()))
                .onItem()
                .ifNull()
       ...
       return Uni.combine()
               .all()
               .unis(countryCheck,emailCheck,userCheck)
               .asTuple()
               .onItem()
               ...
               .transformToUni(user -> userService.persistFlushUser(user))
               .onItem()
               .transform(i->Response.ok().build());
    }

    @RolesAllowed({"Admin","Seller","User"})
    @GET
    @Path("/me")
    public Uni<Response>me(@Context SecurityContext ctx){
       return Panache.withTransaction(()->userService.getByUsername(ctx.getUserPrincipal().getName()))
               .onItem()
               .transform(user->Response.ok().entity(user).type(APPLICATION_JSON_TYPE).build());
    }
}
